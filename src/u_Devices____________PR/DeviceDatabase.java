...
//geben wir das günstigste Gerät zurück
    ...
    // Diese Methode sucht Geräte, die genug Kerne haben – je nach Prozessor-Generation.
// Die Mindestanzahl an Kernen steht in einer Map (z. B. Generation 2 braucht mind. 4 Kerne).
    //Die Methode prüft: „Hat dieses Gerät genug Power (Kerne), passend zur Generation?“
   // Wenn ja → Speichern. Wenn nicht → Ignorieren.

    public ArrayList<Device> findDevicesThatFullfillMinimumPerformance(HashMap<Integer, Integer> minimum) {

        // Wir legen eine leere Liste an, wo wir alle passenden Geräte einsammeln.
        ArrayList<Device> result = new ArrayList<>();

        // Jetzt gehen wir alle Geräte in der Datenbank durch.
        for (Device d : deviceList) {

            // Wir holen uns die Mindestanzahl an Kernen für die Generation des aktuellen Geräts.
            Integer minCores = minimum.get(d.getProcessor().getGeneration());

            // Jetzt prüfen wir: Gibt es überhaupt eine Vorgabe (also minCores ≠ null)
            // UND hat das Gerät genauso viele oder mehr Kerne als gefordert?
            if (minCores != null && d.getProcessor().getNrCores() >= minCores) {

                // Ja, das Gerät erfüllt die Bedingung → wir nehmen es in die Ergebnisliste auf.
                result.add(d);
            }
        }

        // Am Ende geben wir die Liste mit allen passenden Geräten zurück.
        return result;
    }
